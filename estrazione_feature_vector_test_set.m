...
% In questo script si elaborano i segnali della fase di libero uso.
% 
...
for i=1:size(d,1)
    ...
    for stim_type = 1:length(data{1, 2}.classes)

        %inizializzo la matrice che conterrà gli erp medi dei vari canali. I singoli
        %canali sono le colonne della matrice, le righe corrispondono ai
        %diversi campioni dell' erp.
        tipo = replace(data{1, 2}.classes{1, stim_type},"-","_");
        erp_matrix.(tipo) = zeros([durata_erp*data{1,2}.fs/1000,63,1]); %num di campioni di un epoca deve essere 250 poichè fs = 250 Hz.
        %NB: l' ultima dimensione qui è 1 poichè lavoro con un unico soggetto
        %(altrimenti sarebbe length(d).

        %ciclo sui singoli canali
        for ch = 1:length(data{1, 2}.channels)

            %qui vado a mettere i vari ERP di cui vado ad estrarre le
            %variabili temporali 
            erp_tmp = zeros([durata_erp*data{1,2}.fs/1000,1]);   %num di campioni di un epoca deve essere 250 poichè fs = 250 Hz.

            switch stim_type
                case 1
                    idx = find(data{1,2}.y == 1);
                case 2
                    idx = find(data{1,2}.y == 2);
                otherwise
                    error("Errore nello switch");
            end

            trial_onset = data{1,2}.trial(idx); %in questo vettore sono presenti i campioni corrispondenti agli onset degli stimoli

            %scansiono i vari eventi di stimolazione
            for stim_idx = 1:length(trial_onset)
                
                %inizio e fine dell' erp IN CAMPIONI.
                start_erp = trial_onset(stim_idx) - 200*data{1,2}.fs/1000+1; %start a -200ms dall' onset NB: il +1 è fatto per fare combaciare le dimensioni con erp_tmp
                fine_erp = trial_onset(stim_idx) + 800*data{1,2}.fs/1000; %fine a +800ms dall' onset
                
                %% baseline correction

                %erp_tmp = data{1,2}.X(start_erp:fine_erp,ch); %SENZA BASELINE CORRECTION
                erp_tmp = data{1,2}.X(start_erp:fine_erp,ch) - mean(data{1,2}.X(start_erp:trial_onset(stim_idx),ch)); %CON BASELINE CORRECTION
                %% estrazione features
                ...
                for t = 1:size(componenti_erp,1)
                    ...
                    idx_start_interval = round(componenti_erp(t,1)*data{1,2}.fs/1000);
                    idx_end_interval = round(componenti_erp(t,2)*data{1,2}.fs/1000);
...
save("feature vector test set.mat","feature_vector");
